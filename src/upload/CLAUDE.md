...
- **Bounded memory (`MAX_IN_FLIGHT_CHUNKS`)**: The pipeline has *two* channels -- the compression thread's `sync_channel` (`STREAM_CHANNEL_BOUND`) and the tokio bridge to the async upload loop (`BRIDGE_CHANNEL_BOUND`) -- so more than one chunk is live at a time, and the queued chunks are not the only ones. Peak memory for a streamed part is `MAX_IN_FLIGHT_CHUNKS * chunk_size` regardless of part size. Every owner of a `chunk_size` buffer is enumerated in `CHUNK_BUFFER_SLOTS` (stream.rs) -- the queued slots in both channels, the `ChunkedWriter` buffer accumulating the *next* chunk, the compression thread blocked in `send`, the bridge task between `recv` and `blocking_send`, the upload loop's current chunk, and the copy `upload_part_with_retry` clones per attempt. **The constant is derived from both channel bounds**, so neither can be raised without the ceiling moving with it, and `test_in_flight_budget_matches_enumerated_slots` fails if a slot is added to the enumeration without the constant following; a second test asserts the observed in-flight peak stays within the budget. Do not restate the ceiling as a literal number anywhere -- an undercounted ceiling is exactly what that rule exists to prevent.
