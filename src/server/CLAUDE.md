...
`post_actions()` accepts a `Vec<ActionRequest>` body, extracts the first element's `command` field, splits on whitespace, and dispatches based on the first word. For all commands except `delete`, `clean` and `clean_broken`, flags are extracted via `parse_action_flags()` into an `ActionFlags` struct supporting `--table`/`-t`, `--restore-table-mapping` (with automatic database prefix inference from `--table`), `--diff-from-remote`, `--rbac`, `--configs`, and `--rm`/`--drop`. These parsed flags are wired into the corresponding command functions (create, upload, restore, create_remote, restore_remote). Supported commands: `create`, `upload`, `download`, `restore`, `create_remote`, `restore_remote`, `delete`, `clean`, `clean_broken`. `clean` takes an optional name filter via `clean_name_filter()`, accepting both the positional (`clean <name>`) and CLI (`--name <name>` / `--name=<name>`) spellings; it sweeps `shadow/` only and cannot release a deferred freeze, since it holds the `Global` lock while `reap_expired` needs a `Backup(name)` one. Each branch validates backup names, loads config/ch/s3 via `state.config.load()` etc., calls the corresponding command function with default parameters, records duration, and calls `finish_op`/`fail_op`. The backup name defaults to the second word (or UTC timestamp if missing). Returns 400 for unknown commands or empty body, 423 if another operation is running. Uses inline `tokio::select!` for cancellation (not `run_operation()` helper due to incompatible return type).
...
- `system.backup_list` -- SELECT queries proxied to `GET /backup/list` (JSONEachRow, Go-compatible format with `desc` column and `Int64` size)
- `system.backup_actions` -- SELECT/INSERT queries proxied to `GET/POST /backup/actions` (JSONEachRow, Go-compatible status values: `success`/`in progress`/`error`)

On graceful shutdown, both tables are dropped. On startup, existing tables are dropped and recreated to ensure schema/URL updates on upgrade. Creation failures are logged as warnings but do not prevent server startup.
...
- `ListResponse` matches ALL columns of `system.backup_list`. Additionally includes `is_broken: bool` and `broken_reason: Option<String>` fields (with `#[serde(skip_serializing)]`) used internally by the Go-compat layer to populate the `desc` field with `"broken: {reason}"` for broken backups (name, created, location, size, data_size, object_disk_size, metadata_size, rbac_size, config_size, compressed_size, required). All fields are populated from `BackupSummary` via `summary_to_list_response()`: `object_disk_size` from `s.object_disk_size` (sum of S3 object sizes across all parts), `required` from `s.required` (extracted from first `carried:{base}` source in manifest parts), `metadata_size` from `s.metadata_size` (Phase 5), `rbac_size` and `config_size` from `s.rbac_size` and `s.config_size` (Phase 8, computed during `backup::create()`)
