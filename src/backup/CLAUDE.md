...
- **The record is KEPT when an upload fails.** `upload.state.json` survives, so a retry needs the objects still pinned; releasing on failure made every retry unprotected.
...
- **Reaping runs at `create` pre-flight**, not only in `clean`: scheduled deployments run only create and upload, so a clean-only reaper would let orphans accumulate. The reaper acquires and *holds* the per-backup lock across re-read → UNFREEZE → delete, and re-reads after acquiring (a point-in-time check is a TOCTOU gap).
