...
│   ├── clickhouse/
│   │   ├── mod.rs
│   │   └── client.rs       # Connection pool (clickhouse-rs)
│   ├── storage/
│   │   ├── mod.rs
...
| ClickHouse client | §11.3 | `clickhouse-rs` async connection. Test: `SELECT 1`. Connection pool config from `max_connections`. |
...
**Streaming upload pipeline** (§3.6): This is the single most important thing to get right in Phase 1. No temp files.
```
AsyncRead(file) → tar_stream → lz4_encoder → S3 PutObject body
```
Use `tokio::io::AsyncRead` chain. `aws-sdk-s3` `PutObject` accepts `ByteStream` which can wrap an async reader. For Phase 1, single PutObject only (no multipart yet — that's Phase 2).
...
# ClickHouse
clickhouse-rs = "1"           # async CH driver
...
async-compression = { version = "0.4", features = ["tokio", "lz4", "zstd"] }

# Archive
tar = "0.4"
async-tar = "0.5"             # or tokio-tar

# Checksums
crc64fast = "1"               # CRC64 for checksums.txt verification
...
|---|---|---|
| `async-tar` crate immature / missing | Blocks streaming pipeline | Fallback: `spawn_blocking` with sync `tar` crate. Performance hit on small parts only. |
| `clickhouse-rs` missing features | Can't execute some CH commands | Fallback: raw TCP query or HTTP interface for specific commands (FREEZE, SYSTEM). |
