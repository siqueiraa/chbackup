...
Phase 4d adds seven capabilities to the existing phased restore flow:
...
| `--schema` flag (schema-only restore) | Already implemented: CLI flag, restore() parameter, schema_only path in mod.rs. Mode A interaction handled by guard `if rm && !data_only` (schema_only + --rm correctly DROPs then recreates schema without data). No new work needed. | N/A |
| `--data-only` flag (data-only restore) | Already implemented: CLI flag, restore() parameter, data_only guards in mod.rs/schema.rs. Mode A interaction: `--rm --data-only` skips DROP phase (guard `if rm && !data_only`), which is correct since dropping tables then restoring only data would fail. No new work needed. | N/A |
...
- DatabaseReplicated SYNC flag (design §5.1): When database is in `replicated_databases`, append `SETTINGS database_replicated_enforce_synchronous_settings=1` to CREATE TABLE DDL to wait for DDL replication to complete before proceeding. Without this, subsequent operations on other replicas could fail if replication hasn't caught up.
