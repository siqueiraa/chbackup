...
- **S3 concurrency + object_disk_path** (Phase 6): Config fields `s3.concurrency` and `s3.object_disk_path` are parsed from YAML for forward compatibility. `concurrency` controls within-file multipart parallelism; `object_disk_path` provides alternate key prefix for S3 disk objects. (Fields and getters removed from `S3Client` struct as dead code; config parsing retained.)
